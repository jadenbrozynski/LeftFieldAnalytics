...
export async function GET(
  ...
  try {
    ...
    // Query drop and stats separately
    const drop = await queryOne<MatchDropRow>(
      `SELECT id, number, start_date, end_date, created_at FROM match_drops WHERE id = $1`,
      [id]
    )
    ...
    const stats = await queryOne<MatchDropStatsRow>(
      `SELECT * FROM match_drop_stats WHERE match_drop_id = $1`,
      [id]
    )
